"""SIMD-optimized activation functions for AnyTensor and Tensor[dtype].
...
fn _elu_simd_float32(tensor: AnyTensor, mut result: AnyTensor, alpha: Float32):
    ...
    fn vectorized_elu[width: Int](idx: Int) unified {mut}:
        ...
        # SIMD exp may have limited vectorization (Mojo v0.26.1)
...
fn relu_simd_typed[dt: DType](input: Tensor[dt]) raises -> Tensor[dt]:
    """SIMD-optimized ReLU for native Tensor[dtype].

    Uses vectorized max operations directly on typed data pointer.
    No bitcasts needed since Tensor[dt]._data is already typed.

    Args:
        input: Input tensor (typed).

    Returns:
        New Tensor[dt] with ReLU applied element-wise.
    """
    var result = Tensor[dt](input.shape())
    _relu_simd_typed[dt](input, result)
    return result^


fn _leaky_relu_simd_typed[dt: DType](
    input: Tensor[dt], mut result: Tensor[dt], alpha: Scalar[dt]
):
...
fn leaky_relu_simd_typed[dt: DType](
    input: Tensor[dt], alpha: Float64 = 0.01
) raises -> Tensor[dt]:
    """SIMD-optimized Leaky ReLU for native Tensor[dtype].
    ...
    Returns:
        New Tensor[dt] with Leaky ReLU applied element-wise.
    """
    var result = Tensor[dt](input.shape())
    _leaky_relu_simd_typed[dt](input, result, Scalar[dt](alpha))
    return result^


fn _elu_simd_typed[dt: DType](
    input: Tensor[dt], mut result: Tensor[dt], alpha: Scalar[dt]
):
    ...
    fn vectorized_elu[width: Int](idx: Int) unified {mut}:
        ...
        var pos_result = vec
        var neg_clipped = max(vec, SIMD[dt, width](-20.0))
        var exp_result = math_exp(neg_clipped)
        var neg_result = alpha_vec * (exp_result - one_vec)
...
fn elu_simd_typed[dt: DType](
    input: Tensor[dt], alpha: Float64 = 1.0
) raises -> Tensor[dt]:
    """SIMD-optimized ELU for native Tensor[dtype].
    ...
    Returns:
        New Tensor[dt] with ELU applied element-wise.
    """
    var result = Tensor[dt](input.shape())
    _elu_simd_typed[dt](input, result, Scalar[dt](alpha))
    return result^


fn _selu_simd_typed[dt: DType](
    input: Tensor[dt], mut result: Tensor[dt], alpha: Scalar[dt], lambda_: Scalar[dt]
):
    ...
    fn vectorized_selu[width: Int](idx: Int) unified {mut}:
        ...
        var one_vec = SIMD[dt, width](1)
        var alpha_vec = SIMD[dt, width](alpha)
        var lambda_vec = SIMD[dt, width](lambda_)
...
fn selu_simd_typed[dt: DType](
    input: Tensor[dt],
    alpha: Float64 = 1.6732632423543772848170429916717,
    lambda_: Float64 = 1.0507009873554804934193349852946,
) raises -> Tensor[dt]:
    """SIMD-optimized SELU for native Tensor[dtype].
    ...
    Returns:
        New Tensor[dt] with SELU applied element-wise.
    """
    var result = Tensor[dt](input.shape())
    _selu_simd_typed[dt](input, result, Scalar[dt](alpha), Scalar[dt](lambda_))
    return result^


fn _swish_simd_typed[dt: DType](input: Tensor[dt], mut result: Tensor[dt]):
...
fn swish_simd_typed[dt: DType](input: Tensor[dt]) raises -> Tensor[dt]:
    """SIMD-optimized Swish for native Tensor[dtype].

    Args:
        input: Input tensor (typed).

    Returns:
        New Tensor[dt] with Swish applied element-wise.
    """
    var result = Tensor[dt](input.shape())
    _swish_simd_typed[dt](input, result)
    return result^
