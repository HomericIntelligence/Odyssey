...
BEGIN_730EBA86986858EE_THREADS_JSON
[{"thread_id": "PRRT_kwDOQOnaz86OEMS2", "path": "tests/examples/test_mobilenetv1_train_step.mojo", "line": 143, "body": "`compute_gradients` takes `input: AnyTensor` by value. In Mojo, passing a value type moves ownership. After the `compute_gradients` call on this line, `input` may be consumed, making the `model.forward(input, training=False)` call on line 152 a use-after-move compile error. Either reborrow by cloning the tensor before the training step, or restructure so the forward call uses a fresh call to `_make_batch()`. Verify the actual parameter convention of `compute_gradients` (borrowed vs owned)."}]
END_730EBA86986858EE_THREADS_JSON
...
BEGIN_730EBA86986858EE_TODO_LIST
...
END_730EBA86986858EE_TODO_LIST
