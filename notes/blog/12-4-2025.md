...
I've been de-duplicating code this week, and the amount of reduction has been pretty significant, but there still seems to be more code duplication that can be cleaned up. I'm also thinking of pulling ExTensor out into its own repository and just exposing the package and interface to Odyssey just to reduce the amount of changes that occur.
...
- **Evaluation Functions**: Removed duplicate `evaluate()` from `examples/lenet_emnist/` → Created centralized `src/projectodyssey/training/evaluation.mojo`
- **Test Assertions**: Refactored `tests/projectodyssey/conftest.mojo` (reduced by 700+ lines) → Migrated to `src/projectodyssey/testing/assertions.mojo`
- **Gradient Checking**: Consolidated gradient computation logic (reduced by 300 lines) → Moved to `src/projectodyssey/testing/gradient_checker.mojo`
...
- **Deleted `benchmarks/`**: 2,179 lines removed (functionality existed in `src/projectodyssey/benchmarking/`)
- **Deleted `tools/benchmarking/`**: ~266 lines of duplicate benchmark runner
- **Created `src/projectodyssey/data/constants.mojo`**: Centralized dataset lookups (262 new lines of shared code)
...
#### Core Framework (`src/projectodyssey/benchmarking/`)
