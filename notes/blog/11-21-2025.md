...
**Key crash:** Segfault in `src/odyssey/core/conv.mojo:151:64` during backward pass in `compute_gradients()` → `train_epoch()` → `main()`
...
1. **Debug the conv2d segfault** - Fix gradient computation crash at `src/odyssey/core/conv.mojo:151:64`
...
- 1 segfault in `conv2d` during gradient computation (`src/odyssey/core/conv.mojo:151:64`)
