...
```bash
python examples/googlenet_cifar10/download_cifar10.py
```text
...
```bash
mojo run examples/googlenet_cifar10/inference.mojo --weights-dir googlenet_weights
```text
...
- [ ] **Full backward pass**: ~3500 lines — tracked in [GitHub issue #3184](https://github.com/HomericIntelligence/Odyssey/issues/3184)
...
2. **Functional Design**: Uses functional ops from src/odyssey/core
...
```bash
mojo run examples/googlenet_cifar10/inference.mojo \
    --weights-dir googlenet_weights \
    --data-dir datasets/cifar10
```text
