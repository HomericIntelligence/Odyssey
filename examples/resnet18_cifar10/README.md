...
```bash
python examples/resnet18_cifar10/download_cifar10.py
```text
...
mojo run examples/resnet18_cifar10/inference.mojo --weights-dir resnet18_weights
```text
...
normalization backward pass. Once `batch_norm2d_backward` is implemented in `src/odyssey/core/normalization.mojo`, the
...
2. **Functional Design**: Uses functional ops (conv2d, linear, relu, add) from src/odyssey/core
...
```bash
# Test set evaluation
mojo run examples/resnet18_cifar10/inference.mojo \
    --weights-dir resnet18_weights \
    --data-dir datasets/cifar10
```text
