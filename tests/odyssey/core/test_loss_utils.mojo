...
def test_clip_predictions_within_range() raises:
    ...
    var clipped_data = clipped.data_ptr[DType.float32]()
    for i in range(3):
        assert_greater_or_equal(
...
def test_create_epsilon_tensor_values() raises:
    ...
    var eps_data = epsilon_tensor.data_ptr[DType.float32]()
    for i in range(4):
        assert_almost_equal(
...
def test_blend_tensors_mixed() raises:
    ...
    var mask_data = mask.data_ptr[DType.float32]()
...
def main() raises:
    ...
    print("\nAll test_loss_utils tests passed!")
