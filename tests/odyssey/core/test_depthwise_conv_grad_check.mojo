...
def test_depthwise_conv2d_backward_grad_bias_value() raises:
    ...
    assert_almost_equal(
        grads.grad_bias._data.unsafe_bitcast[Float32]()[unsafe_offset=0],
        Float32(0.5),
        tolerance=1e-5,
    )
    assert_almost_equal(
        grads.grad_bias._data.unsafe_bitcast[Float32]()[unsafe_offset=1],
        Float32(-0.3),
        tolerance=1e-5,
    )
...
def test_depthwise_conv2d_backward_full_gradient_pipeline() raises:
    ...
    for i in range(channels * in_h * in_w):
        if grads.grad_input._data.unsafe_bitcast[Float32]()[
            unsafe_offset=i
        ] != Float32(0.0):
            gi_nonzero = True
            break
    ...
    for i in range(channels * kh * kw):
        if grads.grad_weights._data.unsafe_bitcast[Float32]()[
            unsafe_offset=i
        ] != Float32(0.0):
            gk_nonzero = True
            break
    ...
    for i in range(channels):
        if grads.grad_bias._data.unsafe_bitcast[Float32]()[
            unsafe_offset=i
        ] != Float32(0.0):
            gb_nonzero = True
            break
