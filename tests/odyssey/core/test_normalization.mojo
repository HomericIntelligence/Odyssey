...
def _check_grad_input_batch_size(batch_size: Int) raises:
    ...
    if batch_size == 1:
        ...
        for i in range(n_elems):
            var val = grad_input.data_ptr[DType.float32]()[unsafe_offset=i]
            assert_true(
...
def _check_grad_gamma_batch_size(batch_size: Int) raises:
    ...
    if batch_size == 1:
        ...
        for i in range(2):
            var val = grad_gamma.data_ptr[DType.float32]()[unsafe_offset=i]
            assert_true(
...
def _check_grad_beta_batch_size(batch_size: Int) raises:
    ...
    if batch_size == 1:
        # batch_size=1: variance=0 degenerate case — assert finiteness only.
        for i in range(2):
            var val = grad_beta.load[DType.float32](i)
            assert_true(
...
def test_batch_norm2d_zero_variance() raises:
    ...
    for i in range(2):
        var val = output.load[DType.float32](i)
...
def test_batch_norm2d_backward_training_vs_inference() raises:
    ...
    for i in range(8):
        var diff = abs(
            grad_train.load[DType.float32](i)
            - grad_infer.load[DType.float32](i)
        )
        if diff > 1e-5:
            diff_found = True
            break
...
def test_layer_norm_normalization_2d() raises:
    ...
    for i in range(4):
        sum1 += output.load[DType.float32](i)
    var mean1 = sum1 / 4.0
    ...
    for i in range(4, 8):
        sum2 += output.load[DType.float32](i)
    var mean2 = sum2 / 4.0
...
def test_layer_norm_backward_grad_beta() raises:
    ...
    for f in range(4):
        ...
        assert_almost_equal(
            grad_beta.load[DType.float32](f),
            expected_sum,
            tolerance=1e-4,
        )
...
def test_layer_norm_backward_zero_input() raises:
    ...
    for i in range(8):
        var val = grad_input.load[DType.float32](i)
        assert_true(val == val, "grad_input should not be NaN")
        assert_true(val > -1e10 and val < 1e10, "grad_input should be finite")

    for i in range(4):
        var val_gamma = grad_gamma.data_ptr[DType.float32]()[unsafe_offset=i]
        var val_beta = grad_beta.data_ptr[DType.float32]()[unsafe_offset=i]
...
def test_batch_norm2d_backward_gamma_beta_nonzero() raises:
    ...
    assert_almost_equal(
        grad_beta.load[DType.float32](0),
        Float32(8.0),
        tolerance=1e-3,
    )
    assert_almost_equal(
        grad_beta.load[DType.float32](1),
        Float32(8.0),
        tolerance=1e-3,
    )

    # grad_gamma should be non-zero (sum of grad_output * x_hat over N,H,W)
    var gg0 = grad_gamma.load[DType.float32](0)
    var gg1 = grad_gamma.load[DType.float32](1)
