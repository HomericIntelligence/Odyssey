...
def test_conv2d_backward_noncontiguous_grad_output() raises:
    ...
    var bip = bi.data_ptr[DType.float32]()
    var rip = ri.data_ptr[DType.float32]()
    for i in range(bi.numel()):
        assert_almost_equal(
            rip[unsafe_offset=i], bip[unsafe_offset=i], tolerance=1e-4
        )
...
def test_conv2d_backward_noncontiguous_input() raises:
    ...
    var bkp = bk.data_ptr[DType.float32]()
    var rkp = rk.data_ptr[DType.float32]()
    for i in range(bk.numel()):
        assert_almost_equal(
            rkp[unsafe_offset=i], bkp[unsafe_offset=i], tolerance=1e-4
        )
...
def test_conv2d_backward_grad_bias_noncontiguous() raises:
    ...
    var bbp = bb.data_ptr[DType.float32]()
    var rbp = rb.data_ptr[DType.float32]()
    for i in range(bb.numel()):
        assert_almost_equal(
            rbp[unsafe_offset=i], bbp[unsafe_offset=i], tolerance=1e-4
        )
