...
def test_linear_backward_single_sample() raises:
    ...
    assert_almost_equal(
        grad_weights_data[unsafe_offset=1], 2.0, tolerance=1e-5
    )  # [0, 1]
    assert_almost_equal(
        grad_weights_data[unsafe_offset=2], 1.0, tolerance=1e-5
    )  # [1, 0]
    assert_almost_equal(
        grad_weights_data[unsafe_offset=3], 2.0, tolerance=1e-5
    )  # [1, 1]
...
def test_linear_backward_multiple_samples() raises:
    ...
    grad_output_shape.append(out_features)
    var grad_output = ones(grad_output_shape, DType.float32)
    var grad_output_data = grad_output._data.unsafe_bitcast[Float32]()
    grad_output_data[unsafe_offset=0] = 1.0  # grad_output[0, 0]
    grad_output_data[unsafe_offset=1] = 1.0  # grad_output[0, 1]
    grad_output_data[unsafe_offset=2] = 2.0  # grad_output[1, 0]
    grad_output_data[unsafe_offset=3] = 2.0  # grad_output[1, 1]
...
def test_linear_batch_processing() raises:
    ...
    input_data[unsafe_offset=3] = 2.0  # Sample 2: [2, 2]
    input_data[unsafe_offset=4] = 3.0
    input_data[unsafe_offset=5] = 3.0  # Sample 3: [3, 3]
