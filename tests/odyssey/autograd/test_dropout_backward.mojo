...
def test_dropout_backward_p_zero() raises:
    ...
    for i in range(x.numel()):
        assert_almost_equal(
            grad_input._data.unsafe_bitcast[Float32]()[i],
            Float32(1.0),
            tolerance=1e-5,
        )
...
def test_dropout_backward_consistency() raises:
    ...
    for i in range(x.numel()):
        assert_almost_equal(
            grad1._data.unsafe_bitcast[Float32]()[i],
            grad2._data.unsafe_bitcast[Float32]()[i],
            tolerance=1e-6,
        )
...
def test_dropout2d_backward_channel_consistency() raises:
    ...
    for c in range(channels):
        ...
        var channel_mask_val = mask._data.unsafe_bitcast[Float32]()[first_idx]

        # All gradients in this channel should have same pattern
        for h in range(height):
            for w in range(width):
                var idx = c * spatial_size + h * width + w
                var grad_val = grad_input._data.unsafe_bitcast[Float32]()[idx]
