...
def test_sgd_basic_update() raises:
    ...
    # = [0.99, 1.98, 2.97]
    assert_almost_equal(
        Float64(new_params.load[DType.float32](0)),
        0.99,
        tolerance=1e-6,
    )
    assert_almost_equal(
        Float64(new_params.load[DType.float32](1)),
        1.98,
        tolerance=1e-6,
    )
    assert_almost_equal(
        Float64(new_params.load[DType.float32](2)),
        2.97,
        tolerance=1e-6,
    )
...
def test_sgd_momentum_accumulation() raises:
    ...
    assert_almost_equal(
        Float64(params.load[DType.float32](0)), 0.99, tolerance=1e-6
    )
    ...
    assert_almost_equal(
        Float64(params.load[DType.float32](0)),
        0.971,
        tolerance=1e-5,
    )
...
def test_adam_parameter_update() raises:
    ...
    assert_less(params.load[DType.float32](0), 1.0)
    assert_almost_equal(params.load[DType.float32](0), 0.999, tolerance=1e-3)
...
def test_adam_bias_correction() raises:
    ...
    for t in range(1, 6):
        ...
        assert_less(params.load[DType.float32](0), prev_param)
        prev_param = params.load[DType.float32](0)
...
def test_adamw_weight_decay() raises:
    ...
    var param_val = params.load[DType.float32](0)
...
def test_rmsprop_parameter_update() raises:
    ...
    var new_params = result[0]
    var param_value = new_params.load[DType.float32](0)
...
def test_optimizer_property_decreasing_loss() raises:
    ...
    for _ in range(100):
        ...
        var x = params.load[DType.float32](0)
    ...
    var final_x = params.load[DType.float32](0)
...
def test_sgd_matches_pytorch() raises:
    ...
    assert_almost_equal(params.load[DType.float32](0), 0.9900, tolerance=1e-6)
    assert_almost_equal(params.load[DType.float32](1), 1.9800, tolerance=1e-6)
    assert_almost_equal(params.load[DType.float32](2), 2.9700, tolerance=1e-6)
...
def test_adam_matches_pytorch() raises:
    ...
    # Validate against PyTorch (step 1)
    assert_almost_equal(params.load[DType.float32](0), 0.9990, tolerance=1e-4)
    assert_almost_equal(params.load[DType.float32](1), 1.9990, tolerance=1e-4)
    assert_almost_equal(params.load[DType.float32](2), 2.9990, tolerance=1e-4)
...
def test_lion_basic_update_manual() raises:
    ...
    var param_val = new_params.load[DType.float32](0)
...
def test_lion_descent_on_quadratic() raises:
    ...
    for _ in range(num_steps):
        ...
        var grads = zeros(shape, DType.float32)
        for i in range(5):
            var val = params.load[DType.float32](i)
    ...
    var final_val = Float64(params.load[DType.float32](0))
    assert_less(
...
def test_lion_weight_decay() raises:
    ...
    var wd_val = new_params.load[DType.float32](0)
    var no_wd_val = new_params_no_wd.load[DType.float32](0)
    assert_less(
...
def test_shampoo_descent_on_quadratic() raises:
    ...
    for i in range(4):
        var v = Float64(params.load[DType.float32](i))
        initial_norm = initial_norm + v * v

    for _ in range(30):
        var grads = ones(shape, DType.float32)
        for i in range(4):
            var p = params.load[DType.float32](i)
...
def test_shampoo_preconditioner_accumulates() raises:
    ...
    var L_before = Float64(L.load[DType.float32](0))

    var result = shampoo_step(params, grads, L, R, m, learning_rate=0.01)
    var new_L = result[1]
    var L_after = Float64(new_L.load[DType.float32](0))
...
def test_shampoo_weight_decay() raises:
    ...
    for i in range(4):
        initial_sum = initial_sum + Float64(params.load[DType.float32](i))
    ...
    for i in range(4):
        final_sum = final_sum + Float64(params.load[DType.float32](i))
...
def test_shampoo_pure_functional() raises:
    ...
    var param_val_before = Float64(params.load[DType.float32](0))
    var _ = shampoo_step(params, grads, L, R, m, learning_rate=0.01)
    var param_val_after = Float64(params.load[DType.float32](0))
