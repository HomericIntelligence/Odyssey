...
```text
tests/
├── src/odyssey/                          # Shared library tests (primary test suite)
...
**Unit Tests** (`tests/unit/`, `tests/odyssey/core/test_*.mojo`)
- Test individual functions and classes in isolation
- Fast execution (< 1 second each)
- No external dependencies
- Test a single responsibility

**Integration Tests** (`tests/odyssey/integration/`, `tests/integration/`)
...
**Benchmarks** (`tests/odyssey/benchmarks/bench_*.mojo`)
...
**File**: `tests/odyssey/core/test_gradient_checking.mojo`
...
3. **Shared Integration** - Library-level workflows
   - Path: `tests/odyssey/integration/`
...
| --- | --- | --- | --- | --- |
| 1 | Core: Tensors & Operations | `tests/odyssey/core` | ~20 | Tensor creation, arithmetic, reductions |
| 2 | Core: Layers & Activations | `tests/odyssey/core` | ~15 | Fully-connected, ReLU, Sigmoid, etc. |
| 3 | Core: Advanced Layers | `tests/odyssey/core` | ~12 | Conv2D, Pooling, Initialization |
| 4 | Core: Legacy Tests | `tests/odyssey/core/legacy` | ~18 | Historical test suite, edge cases |
| 5 | Training: Optimizers & Schedulers | `tests/odyssey/training` | ~18 | SGD, Adam, RMSprop, learning rate schedules |
| 6 | Training: Loops & Metrics | `tests/odyssey/training` | ~15 | Training/validation loops, accuracy, confusion matrix |
| 7 | Training: Callbacks | `tests/odyssey/training` | ~12 | Checkpointing, early stopping, logging |
| 8 | Data: Datasets | `tests/odyssey/data` | ~10 | TensorDataset, FileDataset, custom datasets |
| 9 | Data: Loaders & Samplers | `tests/odyssey/data` | ~12 | DataLoader, SequentialSampler, WeightedSampler |
| 10 | Data: Transforms | `tests/odyssey/data` | ~15 | Image transforms, augmentation, pipelines |
| 11 | Integration Tests | `tests/odyssey/integration` | ~8 | Full workflow tests |
| 12 | Utils & Fixtures | `tests/odyssey/` | ~12 | Logging, profiling, config, mocks |
| 13 | Benchmarks | `tests/odyssey/benchmarks` | ~4 | Performance measurements |
...
[Parallel] test-gradients.yml (2 min)
  └─ Gradient correctness tests/odyssey/core/test_gradient_checking.mojo
  ↓
[Parallel] integration-tests.yml (8 min)
  ...
  └─ Shared library tests/odyssey/integration/*.mojo
  ↓
...
1. Create file in appropriate subdirectory (e.g., `tests/odyssey/core/test_new_feature.mojo`)
...
```mojo
# tests/odyssey/core/test_new_feature.mojo
...
```mojo
# tests/odyssey/core/test_example.mojo
"""
...
- Use fixtures from `tests/odyssey/fixtures/` for setup
