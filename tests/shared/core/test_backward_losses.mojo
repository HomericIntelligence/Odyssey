...
fn test_cross_entropy_backward_shapes() raises:
    ...
    for i in range(batch):
        targets[i * num_classes] = Float64(1.0)
...
fn test_binary_cross_entropy_backward_shapes() raises:
    ...
    for i in range(batch):
        predictions[i] = Float64(Float32(i) / Float32(batch))
...
fn test_binary_cross_entropy_backward_edge_cases() raises:
    ...
    pred_shape.append(4)
    var predictions = zeros(pred_shape, DType.float32)
    predictions[0] = Float64(0.001)
    predictions[1] = Float64(0.999)
    predictions[2] = Float64(0.5)
    predictions[3] = Float64(0.1)

    var targets = zeros(pred_shape, DType.float32)
    targets[0] = Float64(0.0)
    targets[1] = Float64(1.0)
    targets[2] = Float64(0.0)
    targets[3] = Float64(1.0)
...
fn test_mean_squared_error_backward_shapes() raises:
    ...
    for i in range(batch * features):
        targets[i] = Float64(Float32(i) * 0.1)
...
fn test_mean_squared_error_backward_zero_diff() raises:
    ...
    var predictions = zeros(pred_shape, DType.float32)
    for i in range(5):
        predictions[i] = Float64(Float32(i))

    var targets = zeros(pred_shape, DType.float32)
    for i in range(5):
        targets[i] = Float64(Float32(i))
...
fn test_cross_entropy_backward_gradient() raises:
    ...
    logits[0] = Float64(0.5)
    logits[1] = Float64(-0.3)
    logits[2] = Float64(1.2)
    logits[3] = Float64(-0.8)
    logits[4] = Float64(0.1)
    logits[5] = Float64(0.7)

    var targets_shape = List[Int]()
    targets_shape.append(batch)
    targets_shape.append(num_classes)
    var targets = zeros(targets_shape, DType.float32)
    targets[0] = Float64(1.0)
    targets[4] = Float64(1.0)
...
fn test_binary_cross_entropy_backward_gradient() raises:
    ...
    pred_shape.append(batch)
    var predictions = zeros(pred_shape, DType.float32)
    predictions[0] = Float64(0.1)
    predictions[1] = Float64(0.3)
    predictions[2] = Float64(0.5)
    predictions[3] = Float64(0.7)
    predictions[4] = Float64(0.9)
    predictions[5] = Float64(0.2)
    predictions[6] = Float64(0.6)
    predictions[7] = Float64(0.8)
    ...
    targets[2] = Float64(0.0)
    targets[3] = Float64(1.0)
    targets[4] = Float64(0.0)
    targets[5] = Float64(1.0)
    targets[6] = Float64(0.0)
    targets[7] = Float64(1.0)
...
fn test_mean_squared_error_backward_gradient() raises:
    ...
    pred_shape.append(features)
    var predictions = zeros(pred_shape, DType.float32)
    predictions[0] = Float64(0.5)
    predictions[1] = Float64(-0.3)
    predictions[2] = Float64(1.2)
    predictions[3] = Float64(-0.8)
    predictions[4] = Float64(0.1)
    predictions[5] = Float64(0.7)
    predictions[6] = Float64(2.0)
    predictions[7] = Float64(-1.5)
    predictions[8] = Float64(0.0)
    predictions[9] = Float64(1.0)
    predictions[10] = Float64(-0.5)
    predictions[11] = Float64(0.3)

    var targets = zeros(pred_shape, DType.float32)
    targets[0] = Float64(0.2)
    targets[1] = Float64(0.4)
    targets[2] = Float64(0.8)
    targets[3] = Float64(-0.3)
    targets[4] = Float64(0.5)
    targets[5] = Float64(1.0)
    targets[6] = Float64(1.5)
    targets[7] = Float64(-1.0)
    targets[8] = Float64(0.3)
    targets[9] = Float64(0.7)
    targets[10] = Float64(0.0)
    targets[11] = Float64(0.6)
