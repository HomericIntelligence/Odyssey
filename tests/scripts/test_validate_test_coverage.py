...
class TestGroupSplitFiles:
    ...
    def test_multiple_parts_grouped_together(self) -> None:
        ...
        files = [Path(f"tests/odyssey/core/test_elementwise_dispatch_part{i}.mojo") for i in range(1, 7)]
        groups = group_split_files(files)
        key = "tests/odyssey/core/test_elementwise_dispatch"
    ...
    def test_group_key_preserves_directory(self) -> None:
        ...
        files = [Path("tests/odyssey/core/test_ops_part1.mojo")]
        groups = group_split_files(files)
        assert "tests/odyssey/core/test_ops" in groups
