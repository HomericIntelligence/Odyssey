...
def test_metric_result_tensor() raises:
    ...
    assert_equal(
        value_tensor._data.unsafe_bitcast[Float32]()[0], 0.9, "Tensor value 0"
    )
...
def test_confusion_matrix_integration() raises:
    ...
    var sum = 0
    for i in range(9):
        sum += Int(raw_after_reset._data.unsafe_bitcast[Float64]()[i])
...
def test_metric_interface_consistency() raises:
    ...
    var cm_sum = 0
    for i in range(9):
        cm_sum += Int(cm_after_reset._data.unsafe_bitcast[Float64]()[i])
