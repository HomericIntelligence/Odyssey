...
#
# On hosts with glibc < 2.34 (e.g., Debian 10/Buster), the Mojo binary fails with
# "GLIBC_2.34 not found". This wrapper detects that error and exits 0 (skip)
...
if echo "$output" | grep -q "GLIBC_2\." && echo "$output" | grep -q "not found"; then
    ...
    echo "         Mojo requires GLIBC_2.34+. Your system has an older glibc."
