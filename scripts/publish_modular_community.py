#!/usr/bin/env python3
"""Publish odyssey's conda recipe to modular/modular-community.
...
1. Clones (or refreshes) the HomericIntelligence fork
   ...
   `$HOME/.cache/odyssey/modular-community`. `origin` is the fork;
...
CACHE_DIR = Path.home() / ".cache" / "odyssey" / "modular-community"
...
def copy_recipe(source_root: Path, target_repo: Path, commit_sha: str, dry_run: bool) -> Path:
    """Copy recipe.yaml + test_import.mojo into recipes/odyssey/.
...
Adds the `odyssey` recipe so consumers can install Odyssey's
Mojo library via `pixi add odyssey` from the modular-community
channel.

- **Source**: https://github.com/HomericIntelligence/Odyssey (BSD-3-Clause)
- **Pinned to commit**: `{commit_sha}`

The recipe builds with `mojo package -I src src/odyssey -o ${{PREFIX}}/lib/mojo/odyssey.mojopkg`
...
- [ ] `rattler-build build --recipe recipes/odyssey/recipe.yaml` succeeds
...
def main() -> int:
    ...
    if not (root / "conda.recipe" / "recipe.yaml").exists():
        raise SystemExit(
            f"could not find conda.recipe/recipe.yaml under {root}; run this script from a checkout of Odyssey"
        )

    sha = current_commit_sha(root)
    print(f"Odyssey HEAD: {sha}")

    target = ensure_clone(args.dry_run)
    dest = copy_recipe(root, target, sha, args.dry_run)
    print(f"recipe target: {dest}")

    branch = f"odyssey-recipe-{sha[:7]}"
