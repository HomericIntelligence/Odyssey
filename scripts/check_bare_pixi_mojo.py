    ...
    for workflow_file in workflows_dir.glob("*.yml"):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.search(bare_mojo_pattern, line):
                ...
                if (
                    re.search(r"for\s+\w+\s+in\s+", context)
                    or re.search(r"while\s+\[", context)
                    or re.search(r"attempt=", context)
                    or re.search(r"if\s+pixi\s+run\s+mojo.*then", context)
                ):
                    is_safe = True
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Check for bare pixi run mojo calls in CI workflows")
