...
import argparse, io, sys, tokenize
from pathlib import Path
from token import OP, NAME, COMMENT, NEWLINE, NL, ENCODING, ENDMARKER, INDENT, DEDENT
...
CREATION = {"zeros","ones","full","empty","arange","eye","linspace",
            "ones_like","zeros_like","full_like","nan_tensor","inf_tensor",
            "neg_inf_tensor","randn","_dtype_to_string"}
...
def _validate_partition_table(any_tensor_mojo: Path) -> None:
    ...
    toks = _tokenize(text)
    found = {"tensor_creation": set(), "tensor_utils": set()}
    ...
    for mod, exp in expected.items():
        ...
        if missing or extra:
            drift_msgs.append(
                f"  {mod}: in CREATION/UTILS only={sorted(missing)} "
                f"in any_tensor.mojo only={sorted(extra)}"
            )
    if drift_msgs:
        raise SystemExit(
...
def _partition(symbols: list[str]) -> dict[str, list[str]]:
    buckets = {"any_tensor": [], "tensor_creation": [], "tensor_utils": []}
    for sym in symbols:
        base = sym.split(" as ")[0].strip()
        if base in CREATION:   buckets["tensor_creation"].append(sym)
        elif base in UTILS:    buckets["tensor_utils"].append(sym)
        else:                  buckets["any_tensor"].append(sym)
    return buckets
...
def _iter_mojo_files(root: Path, skip_rel: set[str]) -> list[Path]:
    ...
    for sub in ROOTS:
        ...
        for p in d.rglob("*.mojo"):
            ...
            if any(part in EXCLUDE_PARTS for part in rel_parts): continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--root", type=Path, default=DEFAULT_REPO,
                    help="repo root to sweep (default: script's grandparent)")
    ap.add_argument("--any-tensor-mojo", type=Path, default=None,
    ...
    ap.add_argument("--skip-preflight", action="store_true",
                    help="skip partial-migration preflight (used by tests only)")
    args = ap.parse_args(argv)
    root = args.root.resolve()
    any_tensor_mojo = (args.any_tensor_mojo or
                       root / "src/projectodyssey/tensor/any_tensor.mojo").resolve()
