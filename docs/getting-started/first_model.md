...
- A shared library (`src/odyssey/`) providing `AnyTensor` — a typed, N-dimensional tensor type
- Tensor creation, shape, and element operations in `src/odyssey/core/`
- Data loading utilities in `src/odyssey/data/`
- Training infrastructure in `src/odyssey/training/`
...
The `src/odyssey/` directory contains the core ML components used across all paper implementations.
The entry point is `AnyTensor` in `src/odyssey/core/`.

### Creating Tensors

```mojo
from odyssey.data import TensorDataset, BatchLoader
from odyssey.utils import download_mnist, normalize_images
...
```mojo
from odyssey.utils import load_model, load_image, plot_image
...
```mojo
from odyssey.training import Adam
...
```mojo
from odyssey.data.transforms import RandomRotation, RandomShift
...
- **`src/odyssey/` source** — browse `core/`, `training/`, `data/` subdirectories directly
