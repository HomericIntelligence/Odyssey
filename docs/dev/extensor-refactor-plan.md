...
**Source**: `src/odyssey/core/any_tensor.mojo:798` — `__getitem__` returns `Float32`
**Source**: `src/odyssey/core/any_tensor.mojo:116` — `var _dtype: DType` runtime field
...
- **708 `_data.bitcast[T]()` calls** across the `src/odyssey/` directory
...
**Source**: `src/odyssey/core/any_tensor.mojo:922-992` — 12 `set()` overloads
...
**Source**: `src/odyssey/core/traits.mojo` — already defines trait patterns for the codebase.

### 5.3 Cross-DType Operations

**Problem**: `src/odyssey/training/mixed_precision.mojo` intentionally converts between dtypes:
```mojo
...
**Source**: `src/odyssey/training/mixed_precision.mojo:8,5` — 8 AnyTensor params, 5 returns, 6 dtype branches
...
**Source**: `src/odyssey/base/memory_pool.mojo` — `pooled_alloc` / `pooled_free` byte-level API (moved from `src/odyssey/core/` in Phase 0)

### 5.5 Module/Sequential Stays on AnyTensor

**Problem**: Mojo 0.26.1 doesn't support parametric trait methods. The `Module` trait (`src/odyssey/core/module.mojo:86`) defines `fn forward(mut self, input: AnyTensor) raises -> AnyTensor`. This signature cannot become `fn forward[dt: DType](mut self, input: Tensor[dt]) raises -> Tensor[dt]`.
...
| --- | --- | --- | --- |
| 1 | `src/odyssey/core/any_tensor.mojo` | 263 | The struct itself — every method changes |
| 2 | `src/odyssey/core/elementwise.mojo` | 99 | 27 function signatures change |
| 3 | `src/odyssey/core/activation.mojo` | 79 | 18 dtype branches, 29 return sites |
| 4 | `src/odyssey/core/arithmetic_contiguous.mojo` | 67 | 40 dtype comparisons in 4 functions |
| 5 | `src/odyssey/core/matrix.mojo` | 64 | 48 bitcast calls, partially migrated |
| 6 | `src/odyssey/core/dtype_dispatch.mojo` | 59 | The dispatch infra itself — may be replaced |
| 7 | `src/odyssey/core/arithmetic.mojo` | 58 | 15 function signatures |
| 8 | `src/odyssey/core/numerical_safety.mojo` | 51 | 21 dtype branches, 21 bitcasts |
| 9 | `src/odyssey/core/strassen.mojo` | 40 | 34 bitcasts in 3 long functions |
| 10 | `src/odyssey/core/activation_simd.mojo` | 43 | 24 bitcasts in parametric kernels |
...
- `src/odyssey/core/conv.mojo` — `_conv2d_kernel[dtype: DType]()` pattern
- `src/odyssey/core/elementwise.mojo` — `dispatch_unary`/`dispatch_binary`
- `src/odyssey/core/dtype_dispatch.mojo` — the dispatch infrastructure itself
- `src/odyssey/core/matrix.mojo` — `_matmul_2d_1d_impl[dtype]()` (partially migrated)
...
```
Phase 0:  Package split (src/odyssey/base + src/odyssey/tensor + src/odyssey/core)
    ↓
Phase 1a: Create Tensor[dtype] + TensorLike trait in src/odyssey/tensor/ (additive only)
...
Phase 6:  Training, autograd tape, data pipelines
    ↓
Phase 7a: Tests — src/odyssey/core/ (~180 files)
...
**Files**: `src/odyssey/tensor/tensor.mojo` (NEW), `src/odyssey/tensor/tensor_traits.mojo` (NEW), `src/odyssey/tensor/any_tensor.mojo` (MOVED from `src/odyssey/core/`), `src/odyssey/base/` (NEW — extracted from `src/odyssey/core/`)

**3-Layer Package Architecture** (prerequisite: Phase 0 package split):
```
src/odyssey/base/             # LAYER 1: Zero tensor dependencies
    __init__.mojo
...
src/odyssey/tensor/           # LAYER 2: Imports base only
    __init__.mojo
...
src/odyssey/core/             # LAYER 3: Imports base + tensor
    __init__.mojo
...
Move files from `src/odyssey/core/` to create the 3-layer architecture described above. Each moved file gets updated import paths (`from odyssey.base.X import Y`). `src/odyssey/core/__init__.mojo` and `src/odyssey/__init__.mojo` re-export from new locations for backward compat. ~500 lines of import path updates. This is a mechanical move with zero behavior changes.

#### Phase 1a — Create Tensor[dtype] + TensorLike Trait (additive only)
- Create `src/odyssey/tensor/tensor.mojo`: `struct Tensor[dtype: DType = DType.float32]`
- Create `src/odyssey/tensor/tensor_traits.mojo`: `trait TensorLike(Copyable, Movable)`
...
- Remove `comptime Tensor = AnyTensor` from `src/odyssey/__init__.mojo:82` (see B3)
- Add proper Tensor[dtype] re-export from src/odyssey/tensor/
- Update `src/odyssey/core/__init__.mojo` and `src/odyssey/__init__.mojo` exports
...
#### Phase 1.6 — Implement
- Create `src/odyssey/tensor/` module with `struct Tensor[dtype: DType = DType.float32](TensorLike)`
...
- Rename AnyTensor → AnyTensor in `src/odyssey/core/any_tensor.mojo`, add `as_tensor[dtype]()`, conform to `TensorLike`
- Update `src/odyssey/core/__init__.mojo` and `src/odyssey/__init__.mojo` exports
- Add `comptime AnyTensor = AnyTensor` alias for backward compat
...
**Files**: `src/odyssey/core/tensor.mojo`, `src/odyssey/core/initializers.mojo`
...
#### Phase 7a — Tests: src/odyssey/core/ (~180 files, ~2,500 lines)
...
just test-group "tests/odyssey/core" "test_extensor_setitem.mojo"
just test-group "tests/odyssey/core" "test_creation_part1.mojo"

# Phase 4: Core operations
just test-group "tests/odyssey/core" "test_backward_*.mojo"
just test-group "tests/odyssey/core" "test_arithmetic*.mojo"

# Phase 5: Higher-level
just test-group "tests/odyssey/core" "test_activation*.mojo"
just test-group "tests/odyssey/core" "test_normalization*.mojo"
...
| File | Relevance |
| --- | --- |
| `src/odyssey/core/any_tensor.mojo` | The struct — 4704 lines, 177 dtype branches, 158 bitcasts |
| `src/odyssey/core/dtype_dispatch.mojo` | Existing parametric dispatch infrastructure |
| `src/odyssey/core/dtype_ordinal.mojo` | Ordinal mapping for dispatch fan-out |
| `src/odyssey/core/conv.mojo` | Best example of fully-parametric kernel pattern |
| `src/odyssey/core/arithmetic_contiguous.mojo` | Worst dtype-branching case (40 branches) |
| `src/odyssey/core/normalization.mojo` | Struct-field propagation example |
| `src/odyssey/training/mixed_precision.mojo` | Cross-dtype edge case |
| `src/odyssey/core/traits.mojo` | Existing trait patterns |
| `src/odyssey/core/memory_pool.mojo` | Pool allocation API |
