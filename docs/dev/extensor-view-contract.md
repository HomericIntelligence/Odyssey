...
`transpose_view()` in `src/odyssey/core/matrix.mojo` is **not** a canonical view in the
...
- `src/odyssey/core/any_tensor.mojo` — `reshape()`, `transpose()`, `slice()`
- `src/odyssey/core/shape.mojo` — `broadcast_to()`, `squeeze()`, `unsqueeze()`
- `src/odyssey/core/matrix.mojo` — `transpose_view()` (test utility)
