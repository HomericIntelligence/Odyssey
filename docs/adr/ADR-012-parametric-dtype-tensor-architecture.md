...
in a `src/odyssey/tensor/` package while existing files remain in `src/odyssey/core/`.
...
`src/odyssey/core/any_tensor.mojo:116` (`var _dtype: DType` runtime field),
`src/odyssey/core/any_tensor.mojo:922-992` (12 `set()` overloads)
...
- `src/odyssey/__init__.mojo:82` defines `comptime Tensor = AnyTensor`, creating a naming
...
The new files live in `src/odyssey/tensor/` (tensor.mojo, tensor_traits.mojo). Existing files
remain in `src/odyssey/core/` -- no package reorganization. `comptime AnyTensor = AnyTensor`
...
4. **No package reorganization**: Keeping files in `src/odyssey/core/` avoids 500+ import
