...
def _init_splus_state_single_matrix(
    params: AnyTensor,
) raises -> Tuple[
...
def init_splus_state(
    ...
    *,
    ...
    For rank-2 matrix params emits 6 buffer tensors matching the `_init_splus_state_single_matrix()` tuple. For non-matrix params emits an empty list.
    ...
    for i in range(len(params_list)):
        ...
        if p.ndim() == 2:
            var unpacked = _init_splus_state_single_matrix(p)
