...
def init_adan_state(
    ...
    *,
    ...
    """Allocate per-parameter state buffers for the adan optimizer.

    Returns a `List[List[AnyTensor]]` with outer length == `len(params_list)` (one entry per parameter) and inner length == 4 (one entry per state buffer the optimizer threads across calls).
